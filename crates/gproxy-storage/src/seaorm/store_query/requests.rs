...
impl SeaOrmStorage {
    pub async fn query_upstream_requests(&self, query: &UpstreamRequestQuery) -> Result<Vec<UpstreamRequestQueryRow>, DbErr> {
        ...
        Ok(rows.into_iter().map(|r| UpstreamRequestQueryRow {
            trace_id: r.trace_id,
    ...
    pub async fn count_upstream_requests(&self, query: &UpstreamRequestQuery) -> Result<RequestQueryCount, DbErr> {
        let mut select = upstream_requests::Entity::find();
        if let Scope::Eq(ref v) = query.provider_id {
            select = select.filter(upstream_requests::Column::ProviderId.eq(*v));
        }
    ...
    pub async fn query_downstream_requests(&self, query: &DownstreamRequestQuery) -> Result<Vec<DownstreamRequestQueryRow>, DbErr> {
        ...
        if let Some(from) = query.from_unix_ms {
            select = select.filter(downstream_requests::Column::At.gte(unix_ms_to_offset_datetime(from)));
        }
        if let Some(to) = query.to_unix_ms {
        ...
        let rows = select.all(&self.db).await?;
        let include_body = query.include_body.unwrap_or(false);
        Ok(rows.into_iter().map(|r| DownstreamRequestQueryRow {
            trace_id: r.trace_id,
            at: r.at,
            internal: r.internal,
            user_id: r.user_id,
    ...
    pub async fn count_downstream_requests(&self, query: &DownstreamRequestQuery) -> Result<RequestQueryCount, DbErr> {
        let mut select = downstream_requests::Entity::find();
        if let Scope::Eq(ref v) = query.user_id {
            select = select.filter(downstream_requests::Column::UserId.eq(*v));
        }
