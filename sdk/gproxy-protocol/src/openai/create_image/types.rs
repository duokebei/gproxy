...
mod tests {
    ...
    #[test]
    fn image_model_supports_known_and_custom_values() {
        let known: OpenAiImageModel = serde_json::from_str("\"gpt-image-1\"").unwrap();
        assert_eq!(
            known,
            OpenAiImageModel::Known(OpenAiImageModelKnown::GptImage1)
        );

        let custom: OpenAiImageModel = serde_json::from_str("\"chatgpt-image-latest\"").unwrap();
        assert_eq!(
            custom,
            OpenAiImageModel::Custom("chatgpt-image-latest".to_string())
        );
    }

    #[test]
    fn create_image_response_usage_roundtrip() {
        ...
        let decoded: OpenAiCreateImageResponseBody = serde_json::from_value(payload).unwrap();
        assert_eq!(decoded.usage.unwrap().total_tokens, 46);
    }
}
