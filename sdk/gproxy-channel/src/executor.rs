...
use crate::utils::rewrite::apply_rewrite_rules;
use crate::utils::sanitize::apply_sanitize_rules;
...
/// 2. `ChannelSettings::sanitize_rules()` — body-text regex scrubbing
///    via [`apply_sanitize_rules`]
/// 3. `ChannelSettings::rewrite_rules()` — JSON-path rewrites via
///    [`apply_rewrite_rules`]
///
...
pub fn prepare_for_send<C: Channel>(
    ...
    let sanitize = settings.sanitize_rules();
    let rewrite = settings.rewrite_rules();
    if sanitize.is_empty() && rewrite.is_empty() {
        return Ok(prepared);
    }

    let Ok(mut body_json) = serde_json::from_slice::<serde_json::Value>(&prepared.body) else {
        // Non-JSON body (e.g. multipart upload): leave as-is.
        return Ok(prepared);
    };
    if !sanitize.is_empty() {
        apply_sanitize_rules(&mut body_json, prepared.route.protocol, sanitize);
    }
    if !rewrite.is_empty() {
        apply_rewrite_rules(
            &mut body_json,
