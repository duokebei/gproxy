...
export function RewriteRulesEditor({
  value,
  onChange,
  t,
}: {
  ...
  const [expanded, setExpanded] = useState(false);
  const rules = parseRewriteRules(value);
  ...
  const add = () => {
    commit([...rules, { path: "", action: { type: "Set", value: null } }]);
  ...
  const updateActionType = (idx: number, type: "Set" | "Remove") => {
    ...
    next[idx] = {
      ...next[idx],
      action: type === "Remove" ? { type: "Remove" } : { type: "Set", value: null },
    };
    commit(next);
  };
  ...
  const updateActionValue = (idx: number, raw: string) => {
    ...
    next[idx] = {
      ...next[idx],
      action: { type: "Set", value: parseActionValue(raw) },
    };
    commit(next);
  };
  ...
  const updateFilter = (idx: number, filter: RewriteFilter | undefined) => {
    ...
    next[idx] = { ...next[idx] };
    if (filter) {
      next[idx].filter = filter;
    } else {
      delete next[idx].filter;
    }
    commit(next);
  };
  ...
  const filledCount = rules.filter((r) => r.path.trim() !== "").length;
  ...
  return (
    <CollapsibleSection
      ...
      summary={
        filledCount === 0
          ? t("providers.rewrite.empty")
          : `${filledCount} rule${filledCount > 1 ? "s" : ""}`
      }
      expanded={expanded}
      onToggle={() => setExpanded((v) => !v)}
      expandLabel={t("common.show")}
    ...
    >
      ...
      {rules.length > 0 ? (
        ...
        <p className="py-4 text-center text-xs text-muted">{t("providers.rewrite.empty")}</p>
      )}
    </CollapsibleSection>
  );
}

function RewriteRuleRow({
  rule,
  idx,
  t,
  onUpdatePath,
  ...
  onUpdateActionType: (idx: number, type: "Set" | "Remove") => void;
  onUpdateActionValue: (idx: number, raw: string) => void;
  onUpdateFilter: (idx: number, filter: RewriteFilter | undefined) => void;
  onToggleFilterChip: (idx: number, dimension: "operations" | "protocols", val: string) => void;
  onRemove: (idx: number) => void;
}) {
  const [filterOpen, setFilterOpen] = useState(!!rule.filter);

  return (
    <div className="space-y-2 rounded-lg border border-border bg-panel-muted px-3 py-2">
      {/* Row 1: path + action type + remove */}
      <div className="flex items-start gap-2">
        <div className="flex-1">
          <Input
            value={rule.path}
            onChange={(v) => onUpdatePath(idx, v)}
        ...
        <div className="w-28 shrink-0">
          <Select
            ...
            onChange={(v) => onUpdateActionType(idx, v as "Set" | "Remove")}
            options={[
              { value: "Set", label: "Set" },
      ...
      {/* Row 2: value (only for Set) */}
      {rule.action.type === "Set" ? (
        <TextArea
          value={serializeActionValue(rule.action.value)}
          onChange={(v) => onUpdateActionValue(idx, v)}
          rows={2}
          placeholder={t("providers.rewrite.value_placeholder")}
        />
      ) : null}

      {/* Row 3: filter toggle + filter fields */}
      <div>
        <button
          type="button"
          className="text-xs text-muted hover:text-text"
          onClick={() => {
            if (filterOpen && rule.filter) {
              onUpdateFilter(idx, undefined);
            }
            setFilterOpen((v) => !v);
          }}
        >
          {filterOpen ? "▾" : "▸"} {t("providers.rewrite.filter")}
        </button>

        {filterOpen ? (
          <div className="mt-2 space-y-2 rounded border border-border/50 bg-panel p-2">
            {/* Model pattern */}
            <div>
              <label className="text-[11px] text-muted">{t("providers.rewrite.model_pattern")}</label>
              <Input
                value={rule.filter?.model_pattern ?? ""}
                onChange={(v) => {
                  const current = rule.filter ?? {};
                  const next: RewriteFilter = { ...current, model_pattern: v || undefined };
                  if (!next.model_pattern && !next.operations && !next.protocols) {
                    onUpdateFilter(idx, undefined);
                  } else {
                    onUpdateFilter(idx, next);
                  }
                }}
            ...
            {/* Operations chips */}
            <div>
              <label className="text-[11px] text-muted">{t("providers.rewrite.operations")}</label>
                ...
                {REWRITE_OPERATION_OPTIONS.map((op) => (
                  <button
                    ...
                    className={`btn rounded-full px-2 py-0.5 text-[10px] font-semibold transition ${
                      rule.filter?.operations?.includes(op) ? "btn-primary" : "btn-neutral"
                    }`}
                    onClick={() => onToggleFilterChip(idx, "operations", op)}
                  >
                    {op}
                  </button>
                ))}
            ...
            {/* Protocols chips */}
            <div>
              <label className="text-[11px] text-muted">{t("providers.rewrite.protocols")}</label>
              <div className="mt-1 flex flex-wrap gap-1">
                {REWRITE_PROTOCOL_OPTIONS.map((proto) => (
                  <button
                    ...
                    type="button"
                    className={`btn rounded-full px-2 py-0.5 text-[10px] font-semibold transition ${
                      rule.filter?.protocols?.includes(proto) ? "btn-primary" : "btn-neutral"
                    }`}
                    onClick={() => onToggleFilterChip(idx, "protocols", proto)}
                  >
                    {proto}
                  </button>
                ))}
